...
namespace ProtoList
{
    // 声明并定义 ProtocolMap 为 inline，确保多次包含不重复定义（C++17 起支持）
    inline const std::map<std::string, std::string> ProtocolMap = {
    ...
    // 协议描述查找函数
    inline std::string GetProtoDesc(const std::string& proto)
    {
