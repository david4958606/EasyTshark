...
{
public:
    PacketController(httplib::Server& server, const std::shared_ptr<TsharkManager>& tsharkManager):
        Server(server),
    ...
    void RegisterRoute()
    {
        Server.Post("/api/getPacketList", [this](const httplib::Request& req, httplib::Response& res)
        {
    ...
    void GetPacketList(const httplib::Request& req, httplib::Response& res)
    {
        ...
        do
        {
            ...
            for (auto packet : allPackets)
            {
                ...
                pktObj.AddMember("src_location", rapidjson::Value(packet->SourceLocation.c_str(), allocator),
                                 allocator);
...
private:
    ...
    std::shared_ptr<TsharkManager> TsharkManager;
};
